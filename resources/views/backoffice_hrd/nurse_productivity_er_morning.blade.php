...
<!-- <div class="container"> -->
    <div class="card">        
        ...
        <div class="card-body">
            ...
            <form action="{{ route('nurse_productivity_er_morning_save') }}" method="POST" enctype="multipart/form-data">
                @csrf 
                    <input type="hidden" name="report_date" value="{{ date('Y-m-d') }}">
                    <input type="hidden" name="shift_time" value="เวรเช้า">
                <div class="mb-3">
                    @foreach($shift_morning as $row)
                    <label class="form-label">จำนวนผู้ป่วยเวรเช้า <strong>{{ $row->visit }}</strong> ราย</label>  
