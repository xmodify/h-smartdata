...
<!-- <div class="container"> -->
    <div class="card">        
        ...
        <div class="card-body">
            ...
            <form action="{{ route('nurse_productivity_lr_afternoon_save') }}" method="POST" enctype="multipart/form-data">
                @csrf 
                    <input type="hidden" name="report_date" value="{{ date('Y-m-d',strtotime('-1 day')) }}">
                    <input type="hidden" name="shift_time" value="เวรบ่าย">
                <div class="mb-3">
                    @foreach($shift_afternoon_opd_normal as $row) @foreach($shift_afternoon_opd_high as $row1)
                    <label class="form-label">จำนวนผู้ป่วยนอกในเวร <strong>{{ $row->opd_normal+$row1->opd_high }}</strong> ราย</label>  
                    @endforeach  @endforeach 
                </div>
                <div class="mb-3">
                    @foreach($shift_afternoon_opd_normal as $row)
                ...
                <div class="mb-3">
                    @foreach($shift_afternoon_opd_high as $row)
