@extends('layouts.app')

@section('content')
<div class="container-fluid">
  <div class="row justify-content-center">      
    <div class="col-md-12">
        <form method="POST" enctype="multipart/form-data">
        @csrf
          <div class="row">                          
              <div class="col-md-9" align="left"></div>
              <div class="col-md-2" align="right">     
                  <select class="form-select my-1" name="budget_year">
                  @foreach ($budget_year_select as $row)
                  <option value="{{$row->LEAVE_YEAR_ID}}" @if ($budget_year == "$row->LEAVE_YEAR_ID") selected="selected"  @endif>{{$row->LEAVE_YEAR_NAME}}</option>
                  @endforeach 
                  </select>                        
              </div>
              <div class="col-md-1" align="right">  
                  <button type="submit" class="btn btn-primary my-1 ">{{ __('ค้นหา') }}</button> 
              </div>
          </div>
        </form>
    </div>    
  </div>
</div> 
<!-- row -->           
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-12"> 
            <div class="card">
                <div class="card-header bg-primary bg-opacity-75 text-white">มูลค่ายา 20 อันดับโรค (Primary Diagnosis) ผู้ป่วยใน ปีงบประมาณ {{ $budget_year }} </div>                                                      
                    <table class="table table-bordered table-striped">
                        <thead>
                        <tr class="table-secondary">
                            <th class="text-center">อันดับ</th>
                            <th class="text-center">ชื่อโรค</th>                           
                            <th class="text-center">คน</th>
                            <th class="text-center">ครั้ง</th>
                            <th class="text-center">ต้นทุน</th>
                            <th class="text-center">มูลค่า</th>
                        </tr>     
                        </thead>                 
                        <?php $count = 1 ; ?>
                        @foreach($value_diag_ipd as $row)          
                        <tr>
                            <td align="center">{{ $count }}</td>                   
                            <td align="left">{{ $row->name }}</td>                            
                            <td align="right">{{ number_format($row->hn) }}</td>
                            <td align="right">{{ number_format($row->visit) }}</td>  
                            <td align="right">{{ number_format($row->sum_cost,2) }}</td>   
                            <td align="right">{{ number_format($row->sum_price,2) }}</td>                         
                        </tr>
                        <?php $count++; ?>
                        @endforeach  
                    </table>
            </div>
        </div>          
    </div>
</div>

@endsection

