...
header('Content-Disposition: attachment; filename="Drug_ESRD.xls"');//ชื่อไฟล์
?>
        
<div><strong>ข้อมูลการใช้ยา ESRD ผู้ป่วยนอก วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</strong></div>      
...
<br>
<div><strong>ข้อมูลการใช้ยา ESRD ผู้ป่วยใน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</strong></div>      
    <table class="table table-bordered table-striped">
        <thead>
        <tr class="table-secondary">
            <th class="text-center">สิทธิการรักษา</th>
