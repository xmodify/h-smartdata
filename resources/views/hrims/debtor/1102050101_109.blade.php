...
@section('content')
    <div class="container-fluid">        
        ...
        <div style="overflow-x:auto;">
            <form action="{{ url('hrims/debtor/1102050101_109_delete') }}" method="POST" enctype="multipart/form-data">
                @csrf   
                @method('DELETE')
                <table id="debtor" class="table table-bordered table-striped my-3">
                    ...
                    <tr class="table-success">
                        ...
                        <th class="text-left text-primary" colspan = "7">1102050101.109-ลูกหนี้-ระบบปฏิบัติการฉุกเฉิน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
            ...
            <table class="table table-bordered ">
                ...
                <tr>
                    ...
                    <td align="center">
                        <a class="btn btn-outline-success btn-sm" href="{{ url('hrims/debtor/1102050101_109_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
        ...
        <hr>
        <div style="overflow-x:auto;">
            <form action="{{ url('hrims/debtor/1102050101_109_confirm') }}" method="POST" enctype="multipart/form-data">
                @csrf                
        ...
        @foreach($debtor as $row)
            <div id="receive-{{ $row->vn }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="receive-{{ $row->vn }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form action={{ url('hrims/debtor/1102050101_109/update', $row->vn) }} method="POST">
                        @csrf
                        @method('PUT')
        ...
        function confirmDelete() { 
            ...
            cancelButtonText: 'ยกเลิก'
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050101_109_delete') }}']").submit();
                }
            });
        }
    </script>
    ...
    <script>
        function confirmSubmit() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050101_109_confirm') }}']").submit();
                }
            });
        }
    </script>
