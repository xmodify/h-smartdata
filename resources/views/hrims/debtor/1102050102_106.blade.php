...
@section('content')
    <div class="container-fluid">        
        ...
        <div style="overflow-x:auto;">
            <form action="{{ url('hrims/debtor/1102050102_106_delete') }}" method="POST" enctype="multipart/form-data">
                @csrf   
                @method('DELETE')
                <table id="debtor" class="table table-bordered table-striped my-3">
                    ...
                    <tr class="table-success">
                        ...
                        <th class="text-center" colspan = "9">1102050102.106-ลูกหนี้ค่ารักษา ชําระเงิน OP วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
                    ...
                    <tr>
                        ...
                        <td align="right" width="9%">
                            ...
                            @endif 
                            <a class="btn btn-outline-info btn-sm" href="{{ url('hrims/debtor/1102050102_106/tracking', $row->vn) }}" target="_blank">ติดตาม {{ $row->visit }}</a> 
                        </td>   
            ...
            <table class="table table-bordered ">
                ...
                <tr>
                    <td class="text-primary" align="right">1102050102.106</td>
                    <td class="text-primary" align="left">ลูกหนี้ค่ารักษา ชําระเงิน OP</td>
                    ...
                    <td align="center">
                        <a class="btn btn-outline-success btn-sm" href="{{ url('hrims/debtor/1102050102_106_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
        ...
        <div class="tab-content pt-2" id="myTabContent">
            <div class="tab-pane fade show active" id="pay" role="tabpanel" aria-labelledby="pay-tab">
                <div style="overflow-x:auto;">
                    <form action="{{ url('hrims/debtor/1102050102_106_confirm') }}" method="POST" enctype="multipart/form-data">
                        @csrf                
                        <table id="debtor_search" class="table table-bordered table-striped my-3">
                            ...
                            <tr class="table-secondary">
                                ...
                                <th class="text-center" colspan = "13">ผู้มารับบริการรอชําระเงิน OP วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th>                         
                            </tr>
                            <tr class="table-secondary">
            ...
            <div class="tab-pane fade" id="iclaim" role="tabpanel" aria-labelledby="iclaim-tab">
                <div style="overflow-x:auto;">
                    <form action="{{ url('hrims/debtor/1102050102_106_confirm_iclaim') }}" method="POST" enctype="multipart/form-data"> 
                        @csrf                
        ...
        <div id="receive-{{ $row->vn }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="receive-{{ $row->vn }}" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <form action={{ url('hrims/debtor/1102050102_106/update', $row->vn) }} method="POST">
                    @csrf
                    @method('PUT')
        ...
        function confirmDelete() { 
            ...
            cancelButtonText: 'ยกเลิก'
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050102_106_delete') }}']").submit();
                }
            });
        }
    </script>
<!-- ยืนยันลูกหนี้ -->
    <script>
        function confirmSubmit() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050102_106_confirm') }}']").submit();
                }
            });
        }
    </script>
    <script>
        function confirmSubmit_iclaim() {
            const selected = [...document.querySelectorAll('input[name="checkbox)iclaim[]"]:checked')].map(e => e.value);    
            if (selected.length === 0) {
                Swal.fire('แจ้งเตือน', 'กรุณาเลือกรายการที่จะยืนยัน', 'warning');
                return;
            }
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050102_106_confirm_iclaim') }}']").submit();
                }
            });
        }
    </script>
