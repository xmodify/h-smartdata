...
@section('content')
    <div class="container-fluid">        
        ...
        <div style="overflow-x:auto;">
            <form action="{{ url('hrims/debtor/1102050101_310_delete') }}" method="POST" enctype="multipart/form-data">
                @csrf   
                @method('DELETE')
                <table id="debtor" class="table table-bordered table-striped my-3">
                    ...
                    <tr class="table-success">
                        ...
                        <th class="text-left text-primary" colspan = "11">1102050101.310-ลูกหนี้ค่ารักษา ประกันสังคม ค่าใช้จ่ายสูง IP วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
            ...
            <table class="table table-bordered ">
                ...
                <tr>
                    <td class="text-primary" align="right">1102050101.310</td>
                    ...
                    <td align="center">
                        <a class="btn btn-outline-success btn-sm" href="{{ url('hrims/debtor/1102050101_310_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
        ...
        <hr>
        <div style="overflow-x:auto;">
            <form action="{{ url('hrims/debtor/1102050101_310_confirm') }}" method="POST" enctype="multipart/form-data">
                @csrf                
                <table id="debtor_search" class="table table-bordered table-striped my-3" width="100%">
                    ...
                    <tr class="table-secondary">
                        ...
                        <th class="text-left text-primary" colspan = "17">1102050101.310-ลูกหนี้ค่ารักษา ประกันสังคม ค่าใช้จ่ายสูง IP รอยืนยัน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th>                         
                    </tr>
                    <tr class="table-secondary">
                        ...
                        <th class="text-center">ค่าใช้จ่ายสูง</th>
                        <th class="text-center">ลูกหนี้</th>
                        <th class="text-center">รายการค่าใช้จ่ายสูง</th> 
        ...
        @foreach($debtor as $row)
            <div id="receive-{{ $row->an }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="receive-{{ $row->an }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form action={{ url('hrims/debtor/1102050101_310/update', $row->an) }} method="POST">
                        @csrf
                        @method('PUT')
        ...
        function confirmDelete() { 
            ...
            cancelButtonText: 'ยกเลิก'
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050101_310_delete') }}']").submit();
                }
            });
        }
    </script>
    ...
    <script>
        function confirmSubmit() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.querySelector("form[action='{{ url('hrims/debtor/1102050101_310_confirm') }}']").submit();
                }
            });
        }
    </script>
    ...
    <script>
        ...
        $('#debtor_search').DataTable({
            ...
            buttons: [
                ...
                title: '1102050101.310-ลูกหนี้ค่ารักษา ประกันสังคม ค่าใช้จ่ายสูง IP รอยืนยัน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}'
                }
            ],
            language: {
                search: "ค้นหา:",
