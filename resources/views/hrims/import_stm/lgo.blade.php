...
<div class="container-fluid">
    <div class="row justify-content-center">  
        <div class="col-md-12"> 
            ...
            <div class="card-body">
                <form id="importForm" onsubmit="simulateProcess(event)" action="{{ url('hrims/import_stm/lgo_save') }}" method="POST" enctype="multipart/form-data">
                    @csrf      
                    <div class="row mb-2">            
                        <div class="col"></div>
                            <div class="col-md-5">
                    ...
                    <table id="stm_lgo" class="table table-bordered table-striped my-3">
                        <thead>
                            ...
                            <tr>
                                ...
                                <td align="right">{{ number_format($row->sum_charge_treatment,2) }}</td>
                                <td align="right">{{ number_format($row->sum_adjrw,4) }}</td> 
                                <td align="right">{{ number_format($row->sum_payrate,2) }}</td> 
                                <td align="right">{{ number_format($row->sum_case_iplg,2) }}</td> 
                                <td align="right">{{ number_format($row->sum_case_oplg,2) }}</td>   
                                <td align="right">{{ number_format($row->sum_case_palg,2) }}</td>  
                                <td align="right">{{ number_format($row->sum_case_inslg,2) }}</td>   
                                <td align="right">{{ number_format($row->sum_case_otlg,2) }}</td>  
                                <td align="right">{{ number_format($row->sum_case_pp,2) }}</td> 
                                <td align="right">{{ number_format($row->sum_case_drug,2) }}</td> 
                                <td align="right" class="text-success">{{ number_format($row->sum_compensate_treatment,2) }}</td> 
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('btnSaveReceipt')
                .addEventListener('click', function () {
                    ...
                    fetch("{{ url('hrims/import_stm/lgo_updateReceipt') }}", {
                        method: "POST",
                        headers: {
