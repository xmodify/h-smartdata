...
<div class="container-fluid">
    <div style="overflow-x:auto;">
        <form action="{{ url('finance_debtor/1102050101_304_delete') }}" method="POST" enctype="multipart/form-data">
            @csrf   
            @method('DELETE')
            <table id="debtor" class="table table-bordered table-striped my-3">
                ...
                <tr class="table-success">
                    ...
                    <th class="text-center" colspan = "12">1102050101.304-ลูกหนี้ค่ารักษา ประกันสังคม IP นอกเครือข่าย สังกัด สป.สธ. วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
                ...
                <tr class="table-success">
                    ...
                    <th class="text-center">ค่ารักษาตามสิทธิ</th>  
                ...
                <tr>
                    ...
                    <td align="right">{{ number_format($row->income_pttype,2) }}</td>
                    ...
                    <td align="center">         
                        <button type="button" class="btn btn-outline-warning btn-sm text-primary receive" data-toggle="modal" data-target="#receive-{{ $row->an }}"  data-id="{{ $row->vn }}" > 
                            บันทึกชดเชย
                        </button>
                    </td>   
                ...
                <?php $sum_income_pttype += $row->income_pttype ; ?>
        ...
        <table class="table table-bordered ">
            ...
            <tr>
                <td class="text-primary" align="right">1102050101.304</td>
                <td class="text-primary" align="left">ลูกหนี้ค่ารักษา ประกันสังคม IP นอกเครือข่าย สังกัด สป.สธ.</td>
                <td class="text-primary" align="right">{{ number_format($sum_income,2)}}</td>
                <td class="text-primary" align="right">{{ number_format($sum_income_pttype,2)}}</td>
                ...
                <td align="center">
                    <a class="btn btn-outline-success btn-sm" href="{{ url('finance_debtor/1102050101_304_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
...
<hr>
    <div style="overflow-x:auto;">
        <form action="{{ url('finance_debtor/1102050101_304_confirm') }}" method="POST" enctype="multipart/form-data">
            @csrf                
...
<div id="receive-{{ $row->an }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="receive-{{ $row->an }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <form action={{ url('finance_debtor/1102050101_304/update', $row->an) }} method="POST">
            @csrf
            @method('PUT')
