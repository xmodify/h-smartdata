...
<script>
    function toggle_nonpp(source) {
        checkboxes = document.getElementsByName('checkboxes_nonpp[]');
        for (var i = 0; i < checkboxes.length; i++) {
            checkboxes[i].checked = source.checked;
        }
    }
...
<div class="container-fluid">
    <div style="overflow-x:auto;">
        <form action="{{ url('finance_debtor/1102050101_209_delete') }}" method="POST" enctype="multipart/form-data">
            @csrf   
            @method('DELETE')
            <table id="debtor" class="table table-bordered table-striped my-3">
                ...
                <tr class="table-success">
                    ...
                    <th class="text-center" colspan = "9">1102050101.209-ลูกหนี้ค่ารักษา ด้านการสร้างเสริมสุขภาพและป้องกันโรค (P&P) วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
                ...
                <tr>
                    ...
                    <td align="right">{{ $row->hipdata_code }}-{{ $row->pttype }} [{{ $row->hospmain }}]</td>
        ...
        <table class="table table-bordered ">
            ...
            <tr>
                ...
                <td class="text-primary" align="right">{{ number_format($sum_pp,2)}}</td>
                ...
                <td align="center">
                    <a class="btn btn-outline-success btn-sm" href="{{ url('finance_debtor/1102050101_209_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
...
<hr>
    ...
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="pp-tab" data-bs-toggle="pill" data-bs-target="#pp" type="button" role="tab" aria-controls="pp" aria-selected="true">ICD10-PP</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="nonpp-tab" data-bs-toggle="pill" data-bs-target="#nonpp" type="button" role="tab" aria-controls="nonpp" aria-selected="false">NON ICD10-PP</button>
        </li>
    </ul>
    <!-- Pills Tabs -->
    <div class="tab-content pt-2" id="myTabContent">
        <div class="tab-pane fade show active" id="pp" role="tabpanel" aria-labelledby="pp-tab">
            <div style="overflow-x:auto;">
                <form action="{{ url('finance_debtor/1102050101_209_confirm') }}" method="POST" enctype="multipart/form-data">
                    @csrf                
                    <table id="debtor_search" class="table table-bordered table-striped my-3">
                        ...
                        <tr class="table-secondary">
                            ...
                            <th class="text-center" colspan = "12">ผู้มารับบริการด้านการสร้างเสริมสุขภาพและป้องกันโรค (P&P) ใน CUP วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th> 
                        ...
                        <tr class="table-secondary">
                            ...
                            <th class="text-center">PP</th>                      
                            <th class="text-center">ลูกหนี้</th>
                            <th class="text-center" width="25%">รายการ PPFS</th>
        ...
        <div class="tab-pane fade" id="nonpp" role="tabpanel" aria-labelledby="nonpp-tab">
            <div style="overflow-x:auto;">
                <form action="{{ url('finance_debtor/1102050101_209_confirm_nonpp') }}" method="POST" enctype="multipart/form-data">
                    @csrf                
                    <table id="debtor_search_nonpp" class="table table-bordered table-striped my-3" width="100%">
                        <thead>
                        <tr class="table-secondary">
                            <th class="text-center" width="5%">
                                <button type="submit" class="btn btn-outline-success btn-sm"  onclick="return confirm('ต้องการยืนยันลูกหนี้')">ยืนยันลูกหนี้</button></th>
                            <th class="text-center" colspan = "11">ผู้มารับบริการด้านการสร้างเสริมสุขภาพและป้องกันโรค (P&P) NON ICD10-PP ใน CUP วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th> 
                            <th class="text-center text-success" colspan = "3">FDH</th>                            
                        </tr>
                        <tr class="table-secondary">
                            <th class="text-center"><input type="checkbox" onClick="toggle_nonpp(this)"> All</th>  
                            <th class="text-center" width="6%">วันที่</th>
                            <th class="text-center">Q</th>
                            <th class="text-center">HN</th>
                            <th class="text-center">ชื่อ-สกุล</th>
                        ...
                        <?php $count = 1 ; ?>
                        @foreach($debtor_search_nonpp as $row)
                        <tr>
                            <td class="text-center"><input type="checkbox" name="checkboxes_nonpp[]" value="{{$row->vn}}"></td>                   
                            <td align="center">{{ DateThai($row->vstdate) }} {{ $row->vsttime }}</td>
                            <td align="center">{{ $row->oqueue }}</td>
                            <td align="center">{{ $row->hn }}</td>
                            <td align="left">{{ $row->ptname }}</td>
