...
<div class="container-fluid">
    <div style="overflow-x:auto;">
        <form action="{{ url('finance_debtor/1102050101_203_delete') }}" method="POST" enctype="multipart/form-data">
            @csrf   
            @method('DELETE')
            <table id="debtor" class="table table-bordered table-striped my-3">
                ...
                <tr class="table-success">
                    ...
                    <th class="text-center" colspan = "8">1102050101.203-ลูกหนี้ค่ารักษา UC-OP นอก CUP (ในจังหวัดสังกัด สธ.) วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
        ...
        <table class="table table-bordered ">
            ...
            <tr>
                ...
                <td align="center">
                    <a class="btn btn-outline-success btn-sm" href="{{ url('finance_debtor/1102050101_203_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
...
<hr>
    <div style="overflow-x:auto;">
        <form action="{{ url('finance_debtor/1102050101_203_confirm') }}" method="POST" enctype="multipart/form-data">
            @csrf                
            <table id="debtor_search" class="table table-bordered table-striped my-3">
                ...
                <tr class="table-secondary">
                    ...
                    <th class="text-center" colspan = "9">ผู้มารับบริการ UC-OP นอก CUP (ในจังหวัดสังกัด สธ.) วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th>                         
                    <th class="text-center text-success" colspan = "4">FDH</th>
                </tr>
                <tr class="table-secondary">
                    ...
                    <th class="text-center">กองทุนอื่น</th>                          
                    <th class="text-center">ลูกหนี้</th>
                    <th class="text-center text-success" width="4%">Project</th>
...
<div id="receive-{{ $row->vn }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="receive-{{ $row->vn }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <form action={{ url('finance_debtor/1102050101_203/update', $row->vn) }} method="POST">
            @csrf
            @method('PUT')
