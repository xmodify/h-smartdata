...
<script>
    function toggle_ip(source) {
        checkboxes = document.getElementsByName('checkboxes_ip[]');
        for (var i = 0; i < checkboxes.length; i++) {
            checkboxes[i].checked = source.checked;
        }
    }
...
<div class="container-fluid">
    <div style="overflow-x:auto;">
        <form action="{{ url('finance_debtor/1102050101_307_delete') }}" method="POST" enctype="multipart/form-data">
            @csrf   
            @method('DELETE')
            <table id="debtor" class="table table-bordered table-striped my-3">
                ...
                <tr class="table-success">
                    ...
                    <th class="text-center" colspan = "9">1102050101.307-ลูกหนี้ค่ารักษา ประกันสังคม-กองทุนทดแทน วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</th> 
        ...
        <table class="table table-bordered ">
            ...
            <tr>
                <td class="text-primary" align="right">1102050101.307</td>
                <td class="text-primary" align="left">ลูกหนี้ค่ารักษา ประกันสังคม-กองทุนทดแทน</td>
                ...
                <td align="center">
                    <a class="btn btn-outline-success btn-sm" href="{{ url('finance_debtor/1102050101_307_indiv_excel')}}" target="_blank">ส่งออกรายตัว</a>                
...
<hr>
    ...
    <div class="tab-content pt-2" id="myTabContent">
        <div class="tab-pane fade show active" id="op" role="tabpanel" aria-labelledby="op-tab">
            <div style="overflow-x:auto;">
                <form action="{{ url('finance_debtor/1102050101_307_confirm') }}" method="POST" enctype="multipart/form-data">
                    @csrf                
                    <table id="debtor_search" class="table table-bordered table-striped my-3">
                        ...
                        <tr class="table-secondary">
                            ...
                            <th class="text-center" colspan = "9">ผู้มารับบริการประกันสังคม-กองทุนทดแทน OPD วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th>                         
                        </tr>
                        <tr class="table-secondary">
        ...
        <div class="tab-pane fade" id="ip" role="tabpanel" aria-labelledby="ip-tab">
            <div style="overflow-x:auto;">
                <form action="{{ url('finance_debtor/1102050101_307_confirm_ip') }}" method="POST" enctype="multipart/form-data">
                    @csrf                
                    <table id="debtor_search_ip" class="table table-bordered table-striped my-3" width="100%">
                        ...
                        <tr class="table-secondary">
                            <th class="text-center">
                                <button type="submit" class="btn btn-outline-success btn-sm"  onclick="return confirm('ต้องการยืนยันลูกหนี้')">ยืนยันลูกหนี้</button></th>
                            <th class="text-center" colspan = "15">ผู้มารับบริการประกันสังคม-กองทุนทดแทน IPD วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }} รอยืนยันลูกหนี้</th>                         
                        </tr>
                        <tr class="table-secondary">
                        ...
                        <tr>
                            <td class="text-center"><input type="checkbox" name="checkboxes_ip[]" value="{{$row->vn}}"></td> 
...
<div id="receive-{{ $row->vn }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="receive-{{ $row->vn }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <form action={{ url('finance_debtor/1102050101_307/update', $row->vn) }} method="POST">
            @csrf
            @method('PUT')
