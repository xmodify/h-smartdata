...
<div class="container-fluid">
  <div class="row justify-content-center">  
    <div class="col-md-12">
      <div class="card">
        <div class="card-header bg-primary bg-opacity-75 text-white">ข้อมูลการแก้ไขอุบัติการณ์ความเสี่ยง ส่ง NRLS วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
          <div class="card-body">
            <div class="row">                          
                ...
                <div class="col-md-4" align="right">
                    <a class="btn btn-success my-2 " href="{{ url('backoffice_risk/nrls_editexport') }}" target="_blank" type="submit">
                    Export
                    </a>                    
                </div>      
            </div>
            <table id = "nrls" class="table table-bordered table-striped">
                ...
                <tr class="table-secondary">
                    ...
                    <th class="text-center">วันที่แก้ไขอุบัติการณ์ความเสี่ยง</th>
                    ...
                    <th class="text-center">Keyword/สาเหตุหลัก</th>
                    <th class="text-center">ผลลัพธ์ทางกระบวนการทำงาน</th>
                    <th class="text-center text-danger">RCA/Contributing Factor</th>
                    <th class="text-center">การปรับ/พัฒนาระบบ</th>
                    <th class="text-center">ผลลัพธ์ทางสังคม</th>
                </tr>     
                </thead>
                <?php $count = 1 ; ?> 
                @foreach($nrls as $row)          
                <tr>
                    ...
                    <td align="center">{{ $row->risk_date_edit }}</td> 
                    <td align="center">{{ $row->datadic5 }}</td>    
                    <td align="center">{{ $row->datadic6_name }}-{{ $row->datadic6 }}</td>   
                    <td align="left">{{ $row->risk_detail }}</td> 
                    <td align="left">{{$row->risk_detail_edit}}</td> 
                    <td align="center">{{ $row->risk_detail_edit }}</td> 
                    <td align="left"></td> 
                    <td align="left">{{ $row->RISKREP_INFER_IMPROVE }}</td> 
                    <td align="center">{{ $row->risk_detail_group }}</td> 
                </tr>
                <?php $count++; ?>
                @endforeach          
              </table>        
