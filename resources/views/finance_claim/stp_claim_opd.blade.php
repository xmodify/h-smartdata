...
<div class="container-fluid">
    <div class="card border-success">
        <div class="card-header bg-success text-white">บุคคลที่มีปัญหาสถานะและสิทธิ STP ผู้ป่วยนอก [ส่ง FDH] วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
        <div class="card-body">  
            <div style="overflow-x:auto;">               
                <table id="eclaim_fdh" class="table table-bordered table-striped my-3">
                    ...
                    <tr>
                        ...
                        <td align="center">{{ number_format($row->rcpt_money,2) }}</td>
                        <td align="center">{{ number_format($row->income-$row->rcpt_money,2) }}</td>
                        ...
                        <td align="center">{{ number_format($row->receive_total-($row->income-$row->rcpt_money),2) }}</td>
                ...
                <div class="text-center text-primary">
                    <h4>
                        รวมราคาเรียกเก็บทั้งหมด <strong>{{number_format($sum_income-$sum_rcpt_money,2)}} </strong>บาท |
                        ชดเชยทั้งหมด <strong>{{number_format($sum_receive_total,2)}} </strong>บาท |
                        ส่วนต่าง <strong>{{number_format($sum_receive_total-($sum_income-$sum_rcpt_money),2)}} </strong>บาท
                    </h4>
                </div>
                <br>
            </div>  
 ...
 <div class="container-fluid">
    <div class="card border-Secondary">
        <div class="card-header bg-Secondary text-white">บุคคลที่มีปัญหาสถานะและสิทธิ STP ผู้ป่วยนอก [รอยืนยันส่ง FDH] วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
        <div class="card-body">  
            <div style="overflow-x:auto;">            
