...
<div class="container-fluid">
    <div class="card border-success">
        <div class="card-header bg-success text-white">Telehealth / Telemedicine [ส่ง FDH] วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
        <div class="card-body">  
            <div style="overflow-x:auto;">               
                <table id="eclaim_fdh" class="table table-bordered table-striped my-3">
                    ...
                    <tr>
                        ...
                        <td align="center">{{ number_format($row->receive_op,2) }}</td>
                        <td align="center">{{ number_format($row->receive_op-$row->sum_price,2) }}</td>
                        <td align="center">{{ $row->repno }}</td>
                    </tr>
                    <?php $count++; ?>
                    <?php $sum_sum_price += $row->sum_price ; ?>
                    <?php $sum_receive_op += $row->receive_op ; ?>
                    @endforeach
                </table>
                <div class="text-center text-primary">
                    <h4>
                        รวมราคาเรียกเก็บทั้งหมด <strong>{{number_format($sum_sum_price,2)}} </strong>บาท |
                        ชดเชยทั้งหมด <strong>{{number_format($sum_receive_op,2)}} </strong>บาท |
                        ส่วนต่าง <strong>{{number_format($sum_receive_op-$sum_sum_price,2)}} </strong>บาท
                    </h4>
                </div>
                <br>
            </div>  
 ...
 <div class="container-fluid">
    <div class="card border-Secondary">
        <div class="card-header bg-Secondary text-white">Telehealth / Telemedicine [รอยืนยันส่ง FDH] วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
        <div class="card-body">  
            <div style="overflow-x:auto;">            
