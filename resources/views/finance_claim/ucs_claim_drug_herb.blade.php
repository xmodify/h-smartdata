...
<div class="container-fluid">
    <div class="card-body"> 
        <h6 class="text-primary">
            HERB1:ฟ้าทะลายโจร 500mg - แคปซูลละ 0.85 | HERB2:ขมิ้นชัน 500mg - แคปซูลละ  1.00 | HERB3:ประสะมะแว้ง 200mg - 5.28
            HERB4:ยาแก้ไอมะขามป้อม 120ml - ขวดละ 27.90 | HERB5:ยาไพล 30g - หลอดละ 40.77 | HERB6:เถาวัลย์เปรียง 500mg - แคปซูลละ 0.93
            HERB7:ลูกประคบ 200mg (จ่ายกลับบ้านเท่าน้น) -ลูกละ 62.76 | HERB8:ยาธาตุอบเชย 120ml - ขวดละ 16.29 | HERB9:ยาสหัสธารา 500mg - แคปซูลละ 1.16
            <font color=red> สิ้นสุด 28 กุมภาพันธ์ 2568 </font> 
        </h6>
    </div>
</div>
<div class="container-fluid">
    <div class="card border-success">
        <div class="card-header bg-success text-white">ยาสมุนไพร [ส่ง FDH] วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
        <div class="card-body">  
            <div style="overflow-x:auto;">               
                <table id="eclaim_fdh" class="table table-bordered table-striped my-3">
                    ...
                    <tr>
                        ...
                        <td align="center">{{ number_format($row->receive_hc_hc,2) }}</td>
                        <td align="center">{{ number_format($row->receive_hc_hc-$row->sum_price,2) }}</td>
                        <td align="center">{{ $row->repno }}</td>
                    </tr>
                    <?php $count++; ?>
                    <?php $sum_sum_price += $row->sum_price ; ?>
                    <?php $sum_receive_hc_hc += $row->receive_hc_hc ; ?>
                    @endforeach
                </table>
                <div class="text-center text-primary">
                    <h4>
                        รวมราคาเรียกเก็บทั้งหมด {{number_format($sum_sum_price,2)}} บาท |
                        ชดเชยทั้งหมด {{number_format($sum_receive_hc_hc,2)}} บาท |
                        ส่วนต่าง {{number_format($sum_receive_hc_hc-$sum_sum_price,2)}} บาท
                    </h4>
                </div>
                <br>
            </div>  
 ...
 <div class="container-fluid">
    <div class="card border-Secondary">
        <div class="card-header bg-Secondary text-white">ยาสมุนไพร [รอยืนยันส่ง FDH] วันที่ {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</div>
        <div class="card-body">  
            <div style="overflow-x:auto;">            
