...
<!-- <div class="container"> -->
    <div class="card">        
        <h5 class="alert alert-primary text-center"><strong>ระบบบันทึกผลิตภาพทางการพยาบาล<br>แผนกอุบัติเหตุ-ฉุกเฉิน ER</strong></h5> 
        <div class="card-body">
            <div class="row mb-3"> 
            ...
            <form action="{{ route('nurse_productivity_er_afternoon_save') }}" method="POST" enctype="multipart/form-data">
                @csrf 
                    <input type="hidden" name="report_date" value="{{ date('Y-m-d',strtotime('-1 day')) }}">
                    <input type="hidden" name="shift_time" value="เวรบ่าย">
                <div class="mb-3">
                    @foreach($shift_afternoon as $row)
                    <label class="form-label">จำนวนผู้ป่วยเวรบ่าย <strong>{{ $row->visit }}</strong> ราย</label>  
                    <input type="hidden" name="patient_all" value="{{ $row->visit }}">
                    @endforeach 
                </div>  
                <div class="mb-3">
                    @foreach($shift_afternoon as $row)
                    <label class="form-label">จำนวนผู้ป่วย Emergent <strong>{{ $row->Emergent }}</strong> ราย</label>  
                    <input type="hidden" name="emergent" value="{{ $row->Emergent }}">
                    @endforeach 
                </div>  
                <div class="mb-3">
                    @foreach($shift_afternoon as $row)
                    <label class="form-label">จำนวนผู้ป่วย Urgent <strong>{{ $row->Urgent }}</strong> ราย</label>                    
                    <input type="hidden" name="urgent" value="{{ $row->Urgent }}">
                    @endforeach
                </div>  
                <div class="mb-3">
                    @foreach($shift_afternoon as $row)
                    <label class="form-label">จำนวนผู้ป่วย Acute illness <strong>{{ $row->Acute_illness }}</strong> ราย</label>                    
                    <input type="hidden" name="acute_illness" value="{{ $row->Acute_illness }}">
                    @endforeach
                </div>  
                <div class="mb-3">
                    @foreach($shift_afternoon as $row)
                    <label class="form-label">จำนวนผู้ป่วย Non Acute illness <strong>{{ $row->Non_acute_illness }}</strong> ราย</label>                    
