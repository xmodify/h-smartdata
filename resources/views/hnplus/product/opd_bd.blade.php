...
<!-- <div class="container"> -->
    <div class="card">        
        <h5 class="alert alert-primary text-center"><strong>ระบบบันทึกผลิตภาพทางการพยาบาล<br>แผนกผู้ป่วยนอก</strong></h5> 
        <div class="card-body">
            <div class="row mb-3"> 
            ...
            <h5 class="text-primary">วันที่ {{DateThai(date('Y-m-d'))}} <strong>สรุปเวร BD</strong></h5><br>
            <form action="{{ route('nurse_productivity_opd_bd_save') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-3">
                    @foreach($visit as $row)
                    <label class="form-label">จำนวนผู้ป่วย OPD <strong>{{ $row->opd }}</strong> ราย</label>  
                    <input type="hidden" name="opd" value="{{ $row->opd }}">
                    @endforeach 
                </div>  
                <div class="mb-3">
                    @foreach($visit as $row)
                    <label class="form-label">จำนวนผู้ป่วย ARI <strong>{{ $row->ari }}</strong> ราย</label>                    
