...
class ProductORController extends Controller
{
    ...
    {
        $this->middleware('auth')->only(['or_report','or_product_delete']); 
    }

//or_product_delete--------------------------------------------------------------------------------------------------------------------------
    public function or_report(Request $request)
    {
        $start_date = $request->start_date ?: date('Y-m-d', strtotime("first day of this month"));
        $end_date = $request->end_date ?: date('Y-m-d');  
        
        $product=Nurse_productivity_or::whereBetween('report_date',[$start_date, $end_date])
        ...
        $product_asc=Nurse_productivity_or::whereBetween('report_date',[$start_date, $end_date])
        ...
        return view('hnplus.product.or_report',compact('product_summary','product','start_date',
                'end_date','del_product','report_date','morning'));        
    }

//product_delete----------------------------------------------------------------------------------------------------------------
    public function or_product_delete($id)
    {
        $product=Nurse_productivity_or::find($id)->delete();
        return redirect()->route('hnplus.product.or_report')->with('danger', 'ลบข้อมูลเรียบร้อยแล้ว');
    }

//แจ้งเตือนสถานะการณ์สรุปเวรเช้า รัน 16.00 น.---------------------------------------------------------------------------------------------
    public function or_morning_notify()
    {
        ...
        foreach ($notify as $row) {
            ...
            $url = url('hnplus/product/or_morning');
        }
                
    //แจ้งเตือน Telegram

        $message = "🧑‍⚕️งานผู้ป่วยผ่าตัด OR" ."\n"
        ...
        $chat_ids = explode(',', DB::table('nurse_setting')->where('name', 'telegram_chat_id_product_or')->value('value'));
...
//or_morning-------------------------------------------------------------------------------------------------------------
    public function or_morning()
    {
        ...
        return view('hnplus.product.or_morning',compact('shift'));            
    }

//or_morning_save------------------------------------------------------------------------------------------------------------------
    public function or_morning_save(Request $request)
    {
        ...
        // ==============================
        //   คำนวณ Productivity OR
        // ==============================
        $patient_hr = $patient_all * 0.75;
        $nurse_total = $nurse_oncall + $nurse_partime + $nurse_fulltime;
        $nurse_hr = $nurse_total * 7; // OR ใช้ 7 ชั่วโมง
        ...
        // ==============================
        $productivity_or = Nurse_productivity_or::updateOrCreate(
            // 🔑 เงื่อนไขเช็คข้อมูลซ้ำ
            [
