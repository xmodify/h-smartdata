...
class ProductCKDController extends Controller
{
    ...
    {
        $this->middleware('auth')->only(['ckd_report','ckd_product_delete']); 
    }
...
//ckd_product_delete--------------------------------------------------------------------------------------------------------------------------
    public function ckd_report(Request $request)
    {
        ...
        $product=Nurse_productivity_ckd::whereBetween('report_date',[$start_date, $end_date])
        ...
        $product_asc=Nurse_productivity_ckd::whereBetween('report_date',[$start_date, $end_date])
        ...
        return view('hnplus.product.ckd_report',compact('product_summary','product','start_date',
...
//product_delete----------------------------------------------------------------------------------------------------------------
    ...
    {
        $product=Nurse_productivity_ckd::find($id)->delete();
...
//แจ้งเตือนสถานะการณ์สรุปเวรเช้า รัน 16.00 น.---------------------------------------------------------------------------------------------
    public function ckd_morning_notify()
    {
        $notify = DB::connection('hosxp')->select("
            ...
            FROM opd_dep_queue o1, ovst o2 WHERE o1.depcode IN ('031')
    ...
    //แจ้งเตือน Telegram
        ...
        $chat_ids = explode(',', DB::table('nurse_setting')->where('name', 'telegram_chat_id_product_ckd')->value('value'));
