...
class ProductVIPController extends Controller
{
    ...
    {
        $this->middleware('auth')->only(['vip_report','vip_product_delete']); 
    }
...
//vip_product_delete--------------------------------------------------------------------------------------------------------------------------
    public function vip_report(Request $request)
    {
        ...
        $product=Nurse_productivity_vip::whereBetween('report_date',[$start_date, $end_date])
        ...
        $product_asc=Nurse_productivity_vip::whereBetween('report_date',[$start_date, $end_date])
        ...
        return view('hnplus.product.vip_report',compact('product_summary','product','start_date',
...
//product_delete----------------------------------------------------------------------------------------------------------------
    ...
    {
        $product=Nurse_productivity_vip::find($id)->delete();
...
//แจ้งเตือนสถานะการณ์สรุปเวรดึก รัน 08.00 น.---------------------------------------------------------------------------------------------
    public function vip_night_notify()
    {
        ...
        foreach ($notify as $row) {
            ...
            $url = url('hnplus/product/vip_night');
        }
    ...
    //แจ้งเตือน Telegram
        ...
        $chat_ids = explode(',', DB::table('nurse_setting')->where('name', 'telegram_chat_id_product_vip')->value('value'));
...
//vip_night_save--------------------------------------------------------------------------------------------------------------------
    public function vip_night_save(Request $request)
    {
        ...
        // ✅ ส่งข้อความ Telegram
        $token = DB::table('nurse_setting')->where('name', 'telegram_token')->value('value');
        $chat_ids = explode(',', DB::table('nurse_setting')->where('name', 'telegram_chat_id_product_vip_save')->value('value'));
...
//แจ้งเตือนสถานะการณ์สรุปเวรเช้า รัน 16.00 น.---------------------------------------------------------------------------------------------
    public function vip_morning_notify()
    {
        ...
        foreach ($notify as $row) {
            ...
            $critical_ill = $row->critical_ill;
            $severe_type_null = $row->severe_type_null;
            $url = url('hnplus/product/vip_morning');
        }
...
//แจ้งเตือนสถานะการณ์สรุปเวรบ่าย รัน 00.01 น.---------------------------------------------------------------------------------------------
    public function vip_afternoon_notify()
    {
        ...
        foreach ($notify as $row) {
            ...
            $critical_ill = $row->critical_ill;
            $severe_type_null = $row->severe_type_null;
            $url=url('hnplus/product/vip_afternoon'); 
        }  
