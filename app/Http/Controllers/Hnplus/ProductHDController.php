...
class ProductHDController extends Controller
{
    ...
    {
        $this->middleware('auth')->only(['hd_report','hd_product_delete']); 
    }
...
//hd_product_delete--------------------------------------------------------------------------------------------------------------------------
    public function hd_report(Request $request)
    {
        ...
        $product=Nurse_productivity_hd::whereBetween('report_date',[$start_date, $end_date])
        ...
        $product_asc=Nurse_productivity_hd::whereBetween('report_date',[$start_date, $end_date])
        ...
        return view('hnplus.product.hd_report',compact('product_summary','product','start_date',
...
//product_delete----------------------------------------------------------------------------------------------------------------
    ...
    {
        $product=Nurse_productivity_hd::find($id)->delete();
...
//แจ้งเตือนสถานะการณ์สรุปเวรเช้า รัน 16.00 น.---------------------------------------------------------------------------------------------
    public function hd_morning_notify()
    {
        $notify = DB::connection('hosxp')->select("
            ...
            FROM opd_dep_queue o1, ovst o2 WHERE o1.depcode IN ('033')
    ...
    //แจ้งเตือน Telegram
        ...
        $chat_ids = explode(',', DB::table('nurse_setting')->where('name', 'telegram_chat_id_product_hd')->value('value'));
...
//hd_morning_save------------------------------------------------------------------------------------------------------------------
    public function hd_morning_save(Request $request)
    {
        ...
        // ==============================
        //   คำนวณ Productivity HD
        // ==============================
        $patient_hr = $patient_all * 4;   // HD ใช้ 4 ชั่วโมงต่อคน
        $nurse_total = $nurse_oncall + $nurse_partime + $nurse_fulltime;
        $nurse_hr = $nurse_total * 15;    // เวร HD = 15 ชั่วโมง
        $productivity = ($patient_hr * 100) / max(1, $nurse_hr);
        $hhpuos = $patient_hr / max(1, $patient_all);
        $nurse_shift_time = $patient_all * $hhpuos * (1.4 / 15);

        // ==============================
        //   บันทึกข้อมูลลงฐานข้อมูล
        // ==============================
        $productivity_hd = Nurse_productivity_hd::updateOrCreate(
            // 🔑 เงื่อนไขเช็คข้อมูลซ้ำ
            [
                'report_date' => $request->report_date,
                'shift_time'  => $request->shift_time,
            ],
            // ✏️ ข้อมูลที่ใช้ update / create
            [
